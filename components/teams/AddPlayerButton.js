...
const AddPlayerButton = ({players}) => {
  ...
  return (
    <>
    <div>
        <button onClick={toggleDropdown} id="dropdownUsersButton" data-dropdown-toggle="dropdownUsers" data-dropdown-placement="bottom" className="h-[3.5rem] text-white bg-yellow-500 hover:bg-yellow-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2.5 text-center inline-flex items-center dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:focus:ring-yellow-800" type="button">Add Player <svg className="w-4 h-4 ml-2" aria-hidden="true" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></button>
        
        {showDropdown && (

        <div id="" className="z-[15] fixed translate-x-[-3.2rem] bg-white rounded-lg shadow w-60 dark:bg-gray-700">
      ...
      <div className="relative">
        ...
        <input value={searchPlayer} onChange={(e) => setSearchPlayer(e.target.value)} type="text" id="input-group-search" className="block w-full p-2 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search player" />
      </div>
    </div>
        ...
        .map((player) => (
            <li key={player.id}>
                <a onClick={(e) => addPlayerToTeam(e, player)} className="flex cursor-pointer items-center px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                    <Image width={500}
                        height={500} className="w-6 h-6 mr-2 rounded-full" src="/../public/images/blue-shirt.jpg" alt="Player shirt" />
...
{showDropdown && (
  <div onClick={(e) => setShowDropdown(false)} className='z-[10] opacity-50 fixed top-0 left-0 w-[100%] h-[100%]' />
)}
</>
  )
}
